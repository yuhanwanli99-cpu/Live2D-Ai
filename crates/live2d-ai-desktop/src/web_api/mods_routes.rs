...
//!
//! 职责：暴露 `GET /api/v1/mods` 与 `POST /api/v1/mods/{id}/{action}`
//! （action ∈ enable/disable/restart），把 host 端 `ModRegistry` 的运行状态
//! 透出给前端。
//!
//! # 安全边界
//!
//! - GET /api/v1/mods：只读，Origin 校验不强制（CSRF 风险为 0）。
//! - POST enable/disable/restart：mutating，需 loopback Origin + application/json；
...
/// 列举全部 Mod 状态：`{"mods":[{"id","name","version","api_version","status","enabled"}]}`。
