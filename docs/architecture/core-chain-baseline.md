...
`crates/l2d-wasm-demo/src/`：
- `main.rs`：`"action-state"` 消息处理（约 36 行）；
- `web/surface.rs`：`ActiveAction`、`action_frames()`、`choreography_total_ms()`
  及一整组测试（约 150 行）。

**为什么不删**（这是一个明确的工程判断，不是遗漏）：

1. **没有任何东西会再发 `action-state`**（发送方已全部拆除），因此它是**惰性的**
   ——不可能误触发；
2. 它整体位于 `#[cfg(target_arch = "wasm32")] mod web` 之内，
   **原生 `cargo test` 覆盖不到**（`web` 模块被 wasm 门控），
   改它只能靠 wasm 重建 + 人工看画面来验证；
3. 而它的爆炸半径是**舞台不渲染**（核心链路第 9 环）——为一个零收益的删除
   去冒这个风险不划算。

⇒ 若将来要删：必须 `wasm` 重建 + 肉眼确认皮套仍正常渲染与口型仍动。

### 3.4 待机生命体征：**必须保留**（不是动作系统）

`surface.rs` 的 `IdleState` / `idle_enabled`（呼吸 / 眨眼 / 微表情，
RM6 待机层）是**另一套机制**，与 3.3 的动作动画只是共用 override 层。
前端「外观与互动 → 待机小动作」开关就是它。
**删动作残件时绝不要连带删掉它。**
